...
#[derive(Debug)]
pub struct Pipeline {
    draw_brush: RefCell<elysium_gl_glyph::GlyphBrush>,
    draw_font_map: RefCell<HashMap<String, elysium_gl_glyph::FontId>>,
...
impl Pipeline {
    pub fn new(
        ...
        let draw_brush_builder = elysium_gl_glyph::GlyphBrushBuilder::using_font(font.clone())
    ...
    pub fn queue(&mut self, section: elysium_gl_glyph::Section<'_>) {
    ...
    pub fn draw_queued(
        ...
        region: elysium_gl_glyph::Region,
    ) {
        self.draw_brush
            .borrow_mut()
    ...
    pub fn measure(
        ...
        use elysium_gl_glyph::GlyphCruncher;

        let elysium_gl_glyph::FontId(font_id) = self.find_font(font);

        let section = elysium_gl_glyph::Section {
            bounds: (bounds.width, bounds.height),
            text: vec![elysium_gl_glyph::Text {
                text: content,
                scale: size.into(),
                font_id: elysium_gl_glyph::FontId(font_id),
                extra: elysium_gl_glyph::Extra::default(),
            }],
            ..Default::default()
        };
    ...
    pub fn find_font(&self, font: iced_native::Font) -> elysium_gl_glyph::FontId {
        match font {
            iced_native::Font::Default => elysium_gl_glyph::FontId(0),
