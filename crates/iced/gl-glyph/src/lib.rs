//! A fast text renderer for [`elysium_gl`]. Powered by [`glyph_brush`].
//!
...
pub enum GlyphBrush<F = FontArc, H = DefaultSectionHasher> {
    Core {
        pipeline: pipeline::core::Pipeline,
        glyph_brush:
            glyph_brush::GlyphBrush<pipeline::core::Instance, Extra, F, H>,
    },
    Compatibility {
...
impl<F: Font + Sync, H: BuildHasher> GlyphBrush<F, H> {
    ...
    fn process_queued(&mut self, context: &elysium_gl::Context) {
        match self {
            ...
            GlyphBrush::Core {
                ...
                loop {
                    brush_action = glyph_brush.process_queued(
                        ...
                        pipeline::core::Instance::from_vertex,
                    );
...
impl<F: Font, H: BuildHasher> GlyphBrush<F, H> {
    fn new(
        ...
        GlyphBrush::Core {
            pipeline: pipeline::core::Pipeline::new(
                gl,
                cache_width,
                cache_height,
            ),
